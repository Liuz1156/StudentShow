...
namespace SDM.DAL
{
	...
	public partial class WorkTuanDui
	{
		...
		/*public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("WorkID", "WorkTuanDui"); 
		}
        */
		/// <summary>
		/// 是否存在该记录
		...
		public bool Add(SDM.Model.WorkTuanDui model)
		{
            ...
            strSql.Append("tdmc,UserID_1,UserID_1_des,UserID_2,UserID_2_des,UserID_3,UserID_3_des,WorkName,WorkCate,WorkDes,WorkTime,WorkUrl,WorkPicUrl)");
		...
		public SDM.Model.WorkTuanDui DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["UserID_1"]!=null && row["UserID_1"].ToString()!="")
				{
					model.UserID_1=int.Parse(row["UserID_1"].ToString());
				}
				if(row["UserID_1_des"]!=null)
				{
					model.UserID_1_des=row["UserID_1_des"].ToString();
				}
				if(row["UserID_2"]!=null && row["UserID_2"].ToString()!="")
				{
					model.UserID_2=int.Parse(row["UserID_2"].ToString());
				}
				if(row["UserID_2_des"]!=null)
				{
					model.UserID_2_des=row["UserID_2_des"].ToString();
				}
				if(row["UserID_3"]!=null && row["UserID_3"].ToString()!="")
				{
					model.UserID_3=int.Parse(row["UserID_3"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select WorkID,tdmc,UserID_1,UserID_1_des,UserID_2,UserID_2_des,UserID_3,UserID_3_des,WorkName,WorkCate,WorkDes,WorkTime,WorkUrl,WorkPicUrl ");
			strSql.Append(" FROM WorkTuanDui ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from WorkTuanDui T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "WorkTuanDui";
